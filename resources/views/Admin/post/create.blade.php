                ...
                <div class="flex flex-col w-full">
                    <form action="{{ route('post.store') }}" method="POST" class="p-6">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="body" @if ($errors->has('body')) style="border: 1px solid #c42727" @endif cols="30" placeholder="Body ... " rows="10" class="hadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old("body") }}</textarea>
                            @error('body')
                                <span style="color:#c42727">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <input type="submit" value="Create" class="w-full p-5"
